...
10. **At-most-once business action:** each immutable mandate contract's active usage chain retains up to 256 committed references, rejects duplicates, and then requires rotation into a new replay scope rather than growing forever. `mandateId` is a label, not a global uniqueness key; duplicate active labels make the adapter fail closed.
